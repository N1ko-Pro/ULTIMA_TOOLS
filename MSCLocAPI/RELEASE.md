...
```
npm run release:tool msc-loc-api patch
```
...
ULTIMA_TOOLS clone, pushes it, then pushes the `MSCLoc-API-v<version>` tag that
...
3. Push a matching tag:
   ```
   git tag MSCLoc-API-v<version>
   git push origin MSCLoc-API-v<version>
   ```
   CI builds + tests, then attaches `MSCLocAPI.dll` to the release (prerelease).
4. Confirm the asset URL resolves:
   `https://github.com/N1ko-Pro/ULTIMA_TOOLS/releases/download/MSCLoc-API-v<version>/MSCLocAPI.dll`
